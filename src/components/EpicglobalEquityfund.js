...
const VARdata = [
  ...
  const renderColor12fulLegendText = (value, entry ) => {
    ...
    const DONUTCOLORS9 = ['#1A1549','#9DB1DB','#D0DEF5','#99103B','#B85876','#D296A9','#dcdcdc','#666666','#404040','#262626']
  
  const DONUTCOLORS = [ "#1a2352","#7030a0","#0095c8","#ffc000","#ff9933","#99103b","blue","green","red", "yellow", "orange"];
  const DONUTCOLORS8 = ['#1A1549','#99103b']
  const DONUTCOLORS6 = ['#1A1549','#9DB1DB','#D0DEF5','#99103B','#B85876','#D296A9','#dcdcdc','#666666','#404040','#262626']

const EpicglobalEquity = () => {
...
const [productsMonthlyPerfB, setproductsMonthlyPerfB] = useState([]);
const [productsMonthlyPerfC, setproductsMonthlyPerfC] = useState([]);
...
const [sectorBreakdownSData, setsectorBreakdownSDataState]= useState([]);
...
const[monthlyPerformanceB2DArrayState,setmonthlyPerformanceB2DArrayState]= useState([]);
const[monthsBArrayState,setmonthsBArrayState]= useState([]);

const[monthlyPerformanceC2DArrayState,setmonthlyPerformanceC2DArrayState]= useState([]);
const[monthsCArrayState,setmonthsCArrayState]= useState([]);

const[monthlyPerformanceD2DArrayState,setmonthlyPerformanceD2DArrayState]= useState([]);
const[monthsDArrayState,setmonthsDArrayState]= useState([]);
...
var length = productRegional.length
   ...
   for(let i=1 ; i< length ;i++){
    ...
    let obj={};
    for(let j =0 ;j< productRegional[i].length ;j++){
// if(productRegional[i][j]>0){
      if(!isNaN(productRegional[i][j])){
      let val = productRegional[i][j];
...
console.log("graphDataResponse1")
  ...
  for(let i=0 ; i< productstop10Holding.length ;i++){
   // typeof(products[i][j])
...
// if(productRegional[i][j]>0){
      if(!isNaN(productSectorBreakdown[i][j])){
      let val = productSectorBreakdown[i][j];
      ...
      // console.log(updatedVal);
        ...
        obj[keysVAR[j]] = productSectorBreakdown[i][j] ;
      }     
    // }
      }
      if(obj.value !==0){
        graphDataResponse344.push(obj)
  ...
  setsectorBreakdownSDataState(graphDataResponse344)
  setInceptionArrayState2(InceptionDataArray2)
  setsectorBreakdownState(sectorBreakdownData)
...
const literatureButton=()=>{
  ...
  var LiteratureKeys = ["LiteratureNameKey","FileName","LiteratureDate","LiteratureTitle"];
    ...
    for(let i=0 ; i< Literature.length ;i++){
      ...
      if(FundName == "global_equity_fund"){
...
tt();
  ...
  for(let i=0 ; i< Summary.length ;i++){
    // typeof(products[i][j])
    if(Summary[i].heading== "global_equity_fund"){
  ...
  // var length =  products.length-1;
    for(let i=1 ; i< PRTU.length ;i++){
      ...
      for(let j =0 ;j< PRTU[i].length ;j++){
        ...
        // function useRegex(input) {
          let regex_fund = /[a-zA-Z]+/i;
          // console.log(regex_fund.test(obj.name));
          // console.log("regex.test(input)");
      ...
      // }
        ...
        // function useRegex(input) {
          let RegexHigh = /-/i;
          // console.log(RegexHigh.test(obj.name));
          // console.log("Regex.test(obj.name (-))");
...
var monthsBArr=[];
console.log(productsMonthlyPerfB)
console.log("productsMonthlyPerf")

for(let i= 7; i<=21;i++ ){
  monthsBArr.push(productsMonthlyPerfB[3][i])
}
console.log("months")
...
for(let i =0;i<arrB2D.length;i++){
for(let j =0 ; j< arrB2D[0].length;j++){
...
var monthsCArr=[];
console.log(productsMonthlyPerfC)
console.log("productsMonthlyPerf")

for(let i= 7; i<=21;i++ ){
  monthsCArr.push(productsMonthlyPerfC[3][i])
}
console.log("months")
console.log(monthsBArr)
...
for(let i =0;i<arrC2D.length;i++){
for(let j =0 ; j< arrC2D[0].length;j++){
...
for(let i= 7; i<=21;i++ ){
  monthsDArr.push(productsMonthlyPerfBEUR[3][i])
}
console.log("months")
console.log(monthsBArr)
...
for(let i =0;i<arrD2D.length;i++){
for(let j =0 ; j< arrD2D[0].length;j++){
...
var keys12monthsDisCGBP = ["name", "value"]
 ...
 for(let i=0 ; i< twelvemonPerfCGBP.length ;i++){
  // typeof(products[i][j])
  
  let obj={};
  for(let j =0 ;j< twelvemonPerfCGBP[i].length ;j++){

    if(!isNaN(twelvemonPerfCGBP[i][j])){
    ...
    // console.log(updatedVal);
      ...
      obj[keys12monthsDisCGBP[j]] = twelvemonPerfCGBP[i][j] ;
    }     

    }

    TwelveMperfCGBP.push(obj)
...
console.log(InceptionDataArray);
  ...
  setmonthsBArrayState(monthsBArr)
  setmonthlyPerformanceB2DArrayState(arrB2D)

  setmonthsCArrayState(monthsCArr)
  setmonthlyPerformanceC2DArrayState(arrC2D)

  setmonthsDArrayState(monthsDArr)
  setmonthlyPerformanceD2DArrayState(arrD2D)
...
// const fetchMyAPI = useCallback(() => {
  ...
  fetch(url).then(resp=> resp.json())
   ...
   console.log("count")
    ...
    setproductSectorBreakdown(resp.SectorWeightings)
    setproductsFundinfo(resp.FundSize)
    setproductsShareClass(resp.NAVperShare)
    setproductsMonthlyPerf(resp.MonthlyPerfAGBP)
    setproductsMonthlyPerfB(resp.MonthlyPerfBGBP)
    setproductsMonthlyPerfC(resp.MonthlyPerfCGBP)
    
    setproductsMonthlyPerfBEUR(resp.MonthlyPerfBEUR)

    console.log(resp.MonthlyPerfAGBP);
    console.log("----------------------------")
    // console.log(resp.prtu);
  ...
  if(index ==0){
    ...
    return(
      <tr class="AssetClass__Row-sc-1rmhbx4-5 eVXooJ">
        <th className="align-left">{holding.name}</th>
        <th colspan="1" className="align-right">{holding.value}</th>
      </tr>        
...
function renderCummulativePerformanceNames(cummulatove, index) {
 if(index ==0){
  ...
  return(
        ...
        <th className="align-center">{cummulatove.value}</th>
  
 )}else{
  return(
      
    <th className="align-center">{cummulatove.name}</th>
...
function renderCummulativePerformanceValue(cummulatove, index) {
  if(index==0){
    return(
      ...
      <td className="align-center">{cummulatove.name}</td>
)    
  }else{
  return(


        <td className="align-center">{cummulatove.value}</td>
  )}
}
...
function renderCMSshareinfo(shareinfoparam, index){
  ...
  return(
    <tr className="Shares__Row-sc-1brks4f-2 eoQrEv" key={index}>
      ...
      {/* <td className="align-right">{shareinfoparam.ausdValue}</td> */}
      <td className="align-center">{shareinfoparam.agbpValue}</td>
      {/* <td className="align-right">{shareinfoparam.aeurValue}</td> */}
...
<section className="main-box">
  <div className="container">
      ...
      <div className="accordion__item">
          ...
          <div className="accordion__item">
            ...
            <div className="accordion__item__content block">
              <ul>
                <li><NavLink to="/markets/EpicglobalEquity" className="navlink a a-active-color">EPIC Global Equity Fund <span className="fa fa-angle-right"></span></NavLink></li>
    ...
    <ul class="nav nav-tabs" role="tablist">
      <li class="nav-item nav-item-tabs"> <a class="nav-link active" data-toggle="tab" id="summaryButton" href="#nine" onClick={summaryButton} role="tab">Summary</a> </li>
        ...
        <div class="row chart-row">
          ...
          <div class="col-md-6 VARmargin chart-block"> 
            ...
            <PieChart width={295} height={750} margin ={ {top: -60, right: 50, bottom: 5, left: 10} } >
              ...
              >
                {/* <Label value="website"  position="outside" /> */}
                {graphData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={DONUTCOLORS5[index % DONUTCOLORS5.length]} />
                ))}
                
              </Pie>
              <Label position="inside" dataKey="name" />
          ...
          <PieChart width={295} height={750} margin ={ {top: -60, right: 50, bottom: 5, left: 10} } >
              <Pie
                ...
                label={renderCustomizedLabel}
              >
                {sectorBreakdownSData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={DONUTCOLORS9[index % DONUTCOLORS9.length]} />
                ))}
              </Pie>
              <Tooltip />
              {/* <Legend /> */}
          ...
          <table class="AssetClass__Table-sc-1rmhbx4-3 mt iiGyjE">
            <tbody class="AssetClass__Body-sc-1rmhbx4-4 cyhKrw">
              <tr class="AssetClass__Row-sc-1rmhbx4-5 fQKjoJ">
                <th colspan="99">Market Cap Breakdown</th>
              </tr>
              

              {marketCapState.map(renderMarketState)}
              {/* <tr class="AssetClass__Row-sc-1rmhbx4-5 eVXooJ">
                
                <td>Mega</td><td>50.15%</td></tr><tr class="AssetClass__Row-sc-1rmhbx4-5 eVXooJ"><td>Large</td><td>46.72%</td>
              </tr> */}
            {/*</tbody>
          </table>
                        ...
                        <table class=" table  CumulativePerformance__Table-sc-51pab9-0 hRUkzz hRUkzz_newest">
                            <tbody>
                                ...
                                <tr class="CumulativePerformance__BottomRow-sc-51pab9-2 eeFpGK">
                                  ...
                                  {cummulatovePerfBData.map(renderCummulativePerformanceValue)}
              
                                </tr>
                                <tr class="CumulativePerformance__BottomRow-sc-51pab9-2 eeFpGK">
              
                                  {CummulativeperfeBEURData.map(renderCummulativePerformanceValue)}
                        ...
                        <table class=" table  CumulativePerformance__Table-sc-51pab9-0 hRUkzz hRUkzz_newest">
                            <tbody>
                                ...
                                <tr class="CumulativePerformance__BottomRow-sc-51pab9-2 eeFpGK">
                                  ...
                                  {TwelveMperfBEURState.map(renderCummulativePerformanceValue)}
                        ...
                        <table class=" MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW">
                                <tbody class="table MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    <tr color="#d8e2e8" class="MonthlyPerformance__Row-sc-1n33bhd-4 elqUjA">
                                        ...
                                        {monthsBArrayState.map(renderMonthNames)}
                            ...
                            <table class="table MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW table-striped">
                                <tbody class="table MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    {monthlyPerformanceB2DArrayState.map(render12MonthsData)}
                        ...
                        <table class=" MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW">
                                <tbody class="table MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    <tr color="#d8e2e8" class="MonthlyPerformance__Row-sc-1n33bhd-4 elqUjA">
                                        ...
                                        {monthsCArrayState.map(renderMonthNames)}
                            ...
                            <table class="table MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW table-striped">
                                <tbody class="table MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    {monthlyPerformanceC2DArrayState.map(render12MonthsData)}
                        ...
                        <table class=" MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW">
                                <tbody class="table MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    <tr color="#d8e2e8" class="MonthlyPerformance__Row-sc-1n33bhd-4 elqUjA">
                                        ...
                                        {monthsDArrayState.map(renderMonthNames)}
                            ...
                            <table class="table MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW table-striped">
                                <tbody class="MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    {monthlyPerformanceD2DArrayState.map(render12MonthsData)}
                ...
                <div class="mb-2 row">
                    ...
                    <table class="FundInformation__Table-sc-18irt95-0 cRUpgb">
                            <tbody class=" FundInformation__Body-sc-18irt95-1 hNmQXY">
                                ...
                                <tr className="FundInformation__Row-sc-18irt95-2 fweCQL">
                                  <td className="align-left">Sector</td>   
                                  <td className="align-right">FE – Equity International</td>
                                </tr>
                                <tr className="FundInformation__Row-sc-18irt95-2 fweCQL">
                                ...
                                <tr className="FundInformation__Row-sc-18irt95-2 fweCQL">
                                  <td className="align-left">Preliminary Charge</td>   
                                  <td className="align-right">Up To 5%</td>
                                </tr>
                                <tr className="FundInformation__Row-sc-18irt95-2 fweCQL">
                        ...
                        <table class="table  Shares__Table-sc-1brks4f-0 hBbtmd wealth-table">
                            <tbody class="Shares__Body-sc-1brks4f-1 hbWYKC">
                                ...
                                {ShareClassarray.map(renderShareClassNames)}
                                    ...
                                    <th className="align-left">Share Class</th>
                                    <th className="align-right">A USD</th>
                                    <th className="align-right">A GBP</th>
                                    <th className="align-right">A EUR</th>
                                    <th className="align-right">B USD</th>
                                    <th className="align-right">B GBP</th>
                                    <th className="align-right">B EUR</th>

                                </tr> */}
                                <tr class="Shares__Row-sc-1brks4f-2 hzpAKA">
                                  {/* <th class="align-left">Share Class</th>
                                <th class="align-right">A GBP</th>
                                <th class="align-right">B GBP</th>
                                <th class="align-right">B EUR</th>
                                <th class="align-right">C GBP</th> */}
                                {ShareClassarray.map(renderShareClassNames)}
                                </tr>
                                {/* {ShareClassarray.map(renderShareClassNames)} */}
...
export default EpicglobalEquity;
