...
const EpicipPCCNextGenFunction = () => {
...
const literatureButton=()=>{
  ...
  //       if(obj.email == SessionUserResponse){
  //         console.log(obj.email);
  //         console.log("IF obj.email");
...
const summaryButton=()=>{
  ...
  for(let i=0 ; i< Summary.length ;i++){
    // typeof(products[i][j])
    if(Summary[i].heading== "ss_next_gen_pcc"){
...
const portfolioButton=()=>{
  ...
  var length = CreditRating.length;
   for(let i=1 ; i< length ;i++){
    // typeof(products[i][j])
...
async function fetchSessionUserAPI(){
  ...
  const url = window.location.origin+'/session_data' 
   ...
   .then (resp => {
      ...
      // setSessionUserResponse(resp.email);
...
async function fetchUserAPI(){
  ...
  const Localurl = 'https://epicipprojects.com/pcc_fund_user_data'
  const url = window.location.origin+'/pcc_fund_user_data' 
  ...
  function renderDailyPrices(DailyPrice, index){
    ...
    return(
      <div class="DailyPricing__DailyPriceCard-sc-62f3gi-1 coIgZR">
        <div class="DailyPricing__DailyPrice-sc-62f3gi-2 gsZde">{DailyPrice.symbol}{parseFloat(DailyPrice.Price).toFixed(2)}</div>
            ...
            <div className="accordion__item__content  block">
              <ul>
                ...
                <li><NavLink to="/markets/EpicRNBpcc" className="navlink a ">PCC - Renminbi Bond Fund<span className="fa fa-angle-right"></span></NavLink></li> */}
                                { StateSession !== 0 ?
                <li><NavLink to="/markets/EpicNextGenPCC" className="navlink a a-active-color" >EPIC - Next Generation Bond Fund (PCC)<span className="fa fa-angle-right"></span></NavLink></li>
                   : 
                <li><a href={window.location.origin+"/login_user"} className="navlink a a-active-color">EPIC - Next Generation Bond Fund (PCC)<span className="fa fa-angle-right"></span></a></li> 
                 }
                {/* { StateSession !== 0 ?
                        ...
                        <table class=" MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW">
                                <tbody class="MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV">
                                    ...
                                    <tr class="CumulativePerformance__BottomRow-sc-51pab9-2 eeFpGK">
                                      ...
                                      {PerformanceSummaryState.map(renderCummulativePerformanceNames)}
...
export default EpicipPCCNextGenFunction;
