...
const summaryButton=()=>{
  ...
  for(let i=0 ; i< Summary.length ;i++){
    // typeof(products[i][j])
    if(Summary[i].heading== "ss_nfa_ucits"){
...
// var length =  products.length-1;
  for(let i=5 ; i< PRTU.length ;i++){
    ...
    //     let regex = /[a-zA-Z]+ Gl$1Bd$1Fd$1 UI/i;
    //     return regex.test(input);
    // }
    let regex_UI = /[a-zA-Z]+/i; //Regex for 'NFA'
...
const informationButton=()=>{
  ...
  for(let i=1 ; i< productsFundinfo.length ;i++){
   ...
   for(let j =0 ;j< productsFundinfo[i].length ;j++){
     ...
     let regex = /m/i;
    //  return regex.test(input);
     const kNum = numFormatter(mrounded); // result is million
     console.log(kNum);
     console.log("kNum");
     if(mrounded > 1000000){
...
function rendertop3contri(top3contriparam, index){
  return(
    <tr className="AssetClass__Row-sc-1rmhbx4-5 eVXooJ" key={index}>
      <td className="align-left" style={{height: "31px"}}>{top3contriparam.name}</td>
      <td  className="align-right" style={{height: "31px"}}>{top3contriparam.value}</td>
    </tr>
  )
}
...
function renderbot3contri(bot3contriparam, index){
  ...
  return(
    <tr className="AssetClass__Row-sc-1rmhbx4-5 eVXooJ eVXooJ_newfact" key={index}>
      ...
      <td className="align-left" style={{height: "31.4px"}}>{bot3contriparam.name}</td>
      <td className="align-right" style={{height: "31.4px"}}>{bot3contriparam.value}</td>
    </tr>
  )
}
...
function renderFundinfo(fundinfoparam, index){
  ...
  return(
    <tr className="FundInformation__Row-sc-18irt95-2 fweCQL fweCQL_newFact" key={index}>
      ...
      <td className="align-left" style={{width: "75%"}}>{fundinfoparam.name}</td>
      <td className="align-center" style={{width: "25%"}}>{fundinfoparam.value}</td>
    </tr>
  )
}
    ...
    <div className="row">
      ...
      <div className="accordion__item">
            ...
            <div className="accordion__item__content  block">
              <ul>
                {/* <li><NavLink to="/markets/EpiciNfaUcitsFundRoute" className="navlink a a-active-color">EPIC - NFA Global Bond Fund UI (UCITS)<span className="fa fa-angle-right"></span></NavLink></li> */}
        ...
        <div class="col-md-6 chart-block">
          ...
          <div class="row table-growth-row remove_margin">
            {/*<div class="pr-md-1 col-12 col-md-4 col table-div-margin remove_margin">
                    ...
                    <table class="table  AssetClass__Table-sc-1rmhbx4-3 iiGyjE_dfm iiGyjE_dfmwidth">
                            <tbody class="AssetClass__Body-sc-1rmhbx4-4 cyhKrw">
                    ...
                    <div class="col" >
                        ...
                        <table class=" MonthlyPerformance__Table-sc-1n33bhd-2 bDbyAW">
                                <tbody class="MonthlyPerformance__Body-sc-1n33bhd-3 eLhmcV ">
                                    ...
                                    <tr className="CumulativePerformance__BottomRow-sc-51pab9-2 eeFpGK he-30">

                                      {PerformanceSummaryState.map(renderFundinfo)}
...
export default EpicipNfaUcitsFunction;
